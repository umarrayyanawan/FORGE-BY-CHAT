...
class ToolExecutionRequest(BaseForgeModel):
    ...
    tool_name: str = Field(
        ..., description="Name of the tool: 'github' | 'terminal' | 'docker' | 'deploy'."
    )
    action: str = Field(
        ..., description="Specific action within the tool, e.g. 'create_repo', 'run_tests'."
    )
    ...
    task_id: str | None = Field(default=None, description="Task that triggered this execution.")
...
class ToolExecutionResult(TimestampedModel):
    ...
    duration_ms: int = Field(
        default=0, ge=0, description="Wall-clock execution time in milliseconds."
    )
