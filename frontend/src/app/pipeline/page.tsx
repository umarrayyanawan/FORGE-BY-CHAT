  ...
  if (!projectId) {
    return (
      <div className="mx-auto max-w-2xl px-6 py-10 text-forge-muted">
        No project selected. <a href="/" className="text-forge-accent underline">Start a new project</a>.
      </div>
    );
  }
    ...
    <div className="mx-auto max-w-5xl px-6 py-10">
      <div className="mb-6 flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-forge-muted font-mono">{projectId}</p>
        </div>
        <div className="flex items-center gap-3">
          <div className={`h-2 w-2 rounded-full ${isConnected ? "bg-green-400" : "bg-red-400"}`} />
          <span className="text-xs text-forge-muted">{isConnected ? "Live" : "Disconnected"}</span>
        </div>
      </div>

      <div className="mb-6 grid grid-cols-1 gap-4 sm:grid-cols-3">
        <Card>
          <CardHeader><CardTitle>Current Phase</CardTitle></CardHeader>
          <CardContent>
        ...
        <Card>
          ...
          <CardContent>
            <span className="text-forge-text">
              {pipelineState?.tasks_completed ?? 0} / {pipelineState?.tasks_total ?? 0}
            </span>
          </CardContent>
        </Card>
        <Card>
      ...
      <Card>
        ...
        <CardContent>
          {events.length === 0 ? (
            <p className="text-forge-muted text-sm">Waiting for events...</p>
          ) : (
            <div className="max-h-80 overflow-y-auto space-y-2">
              {events.map((e) => (
                ...
                >
                  ...
                  <span className="text-forge-muted font-mono">{formatDate(e.timestamp)}</span>
                  <span className="text-forge-text truncate">{JSON.stringify(e.payload).slice(0, 80)}</span>
                </div>
              ))}
            </div>
          )}
